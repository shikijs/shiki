...
By default, Shiki loads all available languages, which can be slow. For better performance, specify only the languages you need using the `langs` option.
:::
...
md.use(await Shiki({
  ...
  // Specify only the languages you need for better performance
