...
### `shiki/bundle/react-native`

> [Bundle Size](/guide/#bundle-size): 3.8 MB (minified), 695 KB (gzip), async chunks included

The bundle includes all themes and common web languages, optimized for React Native and Expo environments. It uses the JavaScript regex engine instead of WebAssembly (WASM), making it fully compatible with React Native's JavaScript runtime.

::: info
This bundle is specifically designed for React Native and Expo. It uses `createJavaScriptRegexEngine()` instead of `createOnigurumaEngine()`, which avoids WASM dependencies that don't work in React Native.
:::
