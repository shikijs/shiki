...
it('sVG renderer should generate SVG with correct style attributes', async () => {
...
it('sVG can have 0 corner radius', async () => {
  const r = getSVGRenderer({
    fontFamily: 'IBM Plex Mono',
    bgCornerRadius: 0,
  })
...
it(`sVG renderer should default to minimal background width correctly`, async () => {
  const r = getSVGRenderer({
    fontFamily: 'IBM Plex Mono',
    bgMinWidth: 180,
  })
...
it(`sVG renderer should ignore minimal background width when it's own boundaries are above that`, async () => {
  const r = getSVGRenderer({
    fontFamily: 'IBM Plex Mono',
    bgMinWidth: 150,
  })
